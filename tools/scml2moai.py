...
class SaxHandler (xml.sax.handler.ContentHandler):
	def __init__ (self):
		...
		# modified to contain an array of 2 timeline dictionaries - one for objects and one for bones
		# index 0 for objects, 1 for bones
		self.timelines = {}
		# The index of the current timeline we're working on
		self.timeline = 0
		# The index of the current dict (object or bone) in the current timeline 
		self.subtimeline = 0		
		self.key = 0
	...
	def startElement (self, name, attrs):
		...
		if name == 'folder':
			...
			# if a timeline exists with the id of the object_ref, then use that timeline, otherwise 
			# create an OrderedDict to store a new timeline 
			if not self.timelines.has_key(0):
				self.timelines[0] = OrderedDict()

			keys = self.timelines[0][timeline] if self.timelines[0].has_key(timeline) else OrderedDict()

			# get the z_index
			keys[attrs.getValue('key')] = { 'zindex': attrs.getValue('z_index') }
			# if the object is attached to a bone, then get the bone information too
			keys[attrs.getValue('key')] = { 'parent': attrs.getValue('parent') if attrs.has_key('parent') else None }

			self.timelines[0][timeline] = keys
			
		elif name == 'bone_ref' and self.tags[-3] == "mainline":
			# Similar to the object_ref, except we just want the parent values
			# Get parent from mainline and store by [timesline][key]
			timeline = attrs.getValue('timeline')
			# if a timeline exists with the id of the object_ref, then use that timeline, otherwise 
			# create an OrderedDict to store a new timeline 
			if not self.timelines.has_key(1):
				self.timelines[1] = OrderedDict()

			keys = self.timelines[1][timeline] if self.timelines[1].has_key(timeline) else OrderedDict()

			keys[attrs.getValue('key')] = { 'parent': attrs.getValue('parent') if attrs.has_key('parent') else None }

			self.timelines[1][timeline] = keys

		elif name == "timeline":
		 	self.timeline = attrs.getValue('id') 
		 	self.subtimeline = 1 if attrs.has_key('object_type') and attrs.getValue('object_type') == "bone" else 0
			...
			if self.timelines[self.subtimeline].has_key(self.timeline) and self.timelines[self.subtimeline][self.timeline].has_key(self.key):
				keyframe = self.timelines[self.subtimeline][self.timeline][self.key]
				...
				self.timelines[self.subtimeline][self.timeline][self.key] = keyframe
				
		elif name == "object" and self.tags[-3] == "timeline":
			# retrieve keyframe and populate with data from object tag
			if self.timelines[0].has_key(self.timeline) and self.timelines[0][self.timeline].has_key(self.key):
				keyframe = self.timelines[0][self.timeline][self.key]
				...
				self.timelines[0][self.timeline][self.key] = keyframe

		elif name == "bone" and self.tags[-3] == "timeline":
			# retrieve keyframe and populate with data from bone tag
			if self.timelines[1].has_key(self.timeline) and self.timelines[1][self.timeline].has_key(self.key):				
				# grab bone information (x, y, angle, scale_x, scale_y)
				keyframe = self.timelines[1][self.timeline][self.key]
				...
				keyframe['scale_x'] = attrs.getValue('scale_x') if attrs.has_key('scale_x') else 1
				keyframe['scale_y'] = attrs.getValue('scale_y') if attrs.has_key('scale_y') else 1
				# aparently the bone can have an rgba color too
				
				self.timelines[1][self.timeline][self.key] = keyframe
...
def output_lua(anim_data, outpath, scale):
	...
	for animCnt, animation in enumerate(anim_data.keys()):
		# Generate each animation
		timelines = anim_data[animation]
		f.write('\t[\'%s\'] = {\n' % animation )
		
		for timelineCnt, timeline in enumerate(timelines[0].values()):	
			# Generate Each object layer
			f.write('\t\t[%d] = {\n' % (timelineCnt + 1))
			
			for keyframeCnt, keyframe in enumerate(timeline.values()):
				# Generate each keyframe
				f.write('\t\t\t[%d] = {\n' % (keyframeCnt + 1))
				propertyCnt = 0
				
				# find out if the object has a parent value set
				# if so, get the properties of the corresponding bone so we can adjust
				# the keyframe values

				bone = timelines[1]['parent'][keyframeCnt] if keyframe.has_key('parent') else None

				for anim_property, value in keyframe.items():
					# Generate each property
					propertyCnt += 1
					
					if anim_property in ["x", "y"]:
						# Adjust values for scaled textures and bones
						
						boneValue = bone[anim_property] if boneValue != None else 0

						adjusted_val = int(round(float(value) * scale)) + boneVal
						f.write('\t\t\t\t[\'%s\'] = %s' % (anim_property, adjusted_val))
					
					elif anim_property in ["pivot_x", "pivot_y"]:
					...
					f.write(',\n' if propertyCnt < len(keyframe.keys()) else '\n')
				f.write('\t\t\t},\n' if keyframeCnt+1 < len(timeline) else '\t\t\t}\n')
			f.write('\t\t},\n' if timelineCnt+1 < len(timelines[0]) else '\t\t}\n')
		f.write('\t},\n' if animCnt+1 < len(anim_data.keys()) else '\t}\n')
	f.write('}\n\nreturn anim')
	# Close file
	f.close
